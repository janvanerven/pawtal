<script lang="ts">
  ...
  async function save(publish = false) {
    ...
    try {
      ...
      if (existingPage) {
        await api.admin.updatePage(existingPage.id, payload);
        if (publish) await api.admin.publishPage(existingPage.id);
      } else {
        const created = await api.admin.createPage(payload);
        if (publish) await api.admin.publishPage(created.id);
  ...
  async function loadRevisions() {
    ...
    if (revisionsOpen && revisions.length === 0) {
      ...
      try {
        revisions = await api.admin.getPageRevisions(existingPage.id);
      } catch {
        // silent — revision history is optional
      } finally {
...
<div class="editor-layout">
  ...
  <div class="editor-body">
    ...
    <aside class="editor-sidebar">
      ...
      <!-- Revision history (only for existing pages) -->
      {#if existingPage}
        <div class="sidebar-section card">
          <button
            type="button"
